...
class ScoutStatus {
  constructor(initObj={}) {
    if (initObj === null) {
      ...
      if (initObj.hasOwnProperty('driver_states')) {
        ...
        this.driver_states = new Array(4).fill(new ScoutDriverState());
      }
  ...
  static serialize(obj, buffer, bufferOffset) {
    ...
    if (obj.driver_states.length !== 4) {
      throw new Error('Unable to serialize array field driver_states - length must be 4')
    }
    // Serialize message field [driver_states]
    obj.driver_states.forEach((val) => {
      bufferOffset = ScoutDriverState.serialize(val, buffer, bufferOffset);
    });
  ...
  static deserialize(buffer, bufferOffset=[0]) {
    ...
    for (let i = 0; i < len; ++i) {
      data.driver_states[i] = ScoutDriverState.deserialize(buffer, bufferOffset)
    }
  ...
  static Resolve(msg) {
    ...
    if (msg.driver_states !== undefined) {
      resolved.driver_states = new Array(4)
      for (let i = 0; i < resolved.driver_states.length; ++i) {
        if (msg.driver_states.length > i) {
          resolved.driver_states[i] = ScoutDriverState.Resolve(msg.driver_states[i]);
        }
        else {
          resolved.driver_states[i] = new ScoutDriverState();
        }
      }
    }
    else {
      resolved.driver_states = new Array(4).fill(new ScoutDriverState())
    }
